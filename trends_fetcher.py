...
def enrich_rows(rows):
    ...
    qids_map = batch_lookup_qids(titles)
    qids = set(qids_map.values()) - {None}
    props = batch_get_claims({t:q for t,q in qids_map.items() if q})

    all_s = [id for p in props.values() for id in p['sports']]
    all_l = [id for p in props.values() for id in p['leagues']]
    s_lbl = batch_resolve_labels(all_s)
    l_lbl = batch_resolve_labels(all_l)
    ...
    for row in rows:
        ...
        if not(sp and lg): fs, fl = scrape_infobox(t); sp = sp or fs; lg = lg or fl
